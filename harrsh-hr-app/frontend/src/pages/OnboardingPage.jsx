...
export const onboardingSchema = z.object({
  ...
  workingDays: z.string().min(1, 'At least one working day is required'),
...
const OnboardingPage = () => {
  ...
  const steps = [
    ...
    <div className="space-y-4">
          ...
          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((d, i) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm border ${selected ? 'bg-indigo-600 text-white border-indigo-600' : 'border-gray-300 text-gray-600'}`}
              >
                {d}
              </button>
            );
      ...
      <div>
        <label className={labelCls}>Payroll Day of Month</label>
        <input
          type="number"
          min="1"
          max="31"
          ...
          onChange={(e) => set('payrollDay', parseInt(e.target.value))}
        />
      </div>
    </div>,
    ...
    <div className="space-y-4">
      <div>
        <label className={labelCls}>Leave Approval Levels</label>
        <select
          className={inputCls}
          value={form.leaveApprovalLevels}
          onChange={(e) => set('leaveApprovalLevels', parseInt(e.target.value))}
        >
      ...
      <div>
        <label className={labelCls}>Allow Backdated Leaves (days)</label>
        <input
          type="number"
          min="0"
          max="30"
          className={inputCls}
          value={form.allowLeaveBackdate}
          onChange={(e) => set('allowLeaveBackdate', parseInt(e.target.value))}
        />
      </div>
    </div>,
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-medium text-gray-700">Require GPS for Check-in</p>
          <p className="text-xs text-gray-500">Employees must be near office to check in</p>
        </div>
        <button
          type="button"
          onClick={() => set('requireCheckInGPS', !form.requireCheckInGPS)}
          className={`relative w-12 h-6 rounded-full transition-colors ${form.requireCheckInGPS ? 'bg-indigo-600' : 'bg-gray-300'}`}
        >
          <span
            className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${form.requireCheckInGPS ? 'translate-x-7' : 'translate-x-1'}`}
          />
        </button>
      </div>
      {form.requireCheckInGPS && (
        <>
          <div>
            <label className={labelCls}>Allowed Radius (meters)</label>
            <input
              type="number"
              className={inputCls}
              value={form.gpsRadius}
              onChange={(e) => set('gpsRadius', parseInt(e.target.value))}
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
