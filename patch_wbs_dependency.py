...
# ── 3. Add DEPENDENCY + PLUS headers after PROGRESS ────────────────────────
DEP_HDR = (
...
# ── 4. Add DEPENDENCY + PLUS cells ─────────────────────────────────────────
# Structure: progress-cell-div > [bar, span]
# After the span closes, close the progress-cell-div, then add dep+plus as siblings in the flex row
# OLD: span close `})`, cell-div close `]})`, sep `,`, flex-row close `]})`, sep `,`, `_e&&je.map`
# NEW: span close `})`, cell-div close `]})`, sep `,`, DEP_CELL, flex-row close `]})`, sep `,`, `_e&&je.map`
#
# DEP_CELL uses ternary for children (no IIFE, no spread, Object.assign for state updates)
DEP_CELL = (
    # dep column cell
    'e.jsx("div",{style:{width:100,flexShrink:0,display:"flex",alignItems:"center",'
    'justifyContent:"center",gap:5,cursor:"pointer"},'
    'onClick:()=>_setDepItem(W),'
    'children:(_deps[W.id]&&(_deps[W.id].blocking.length||_deps[W.id].blocked.length))'
    '?e.jsxs(e.Fragment,{children:['
    '_deps[W.id].blocking.length>0&&e.jsxs("span",{style:{fontSize:10,fontWeight:700,color:"#d97706",'
    'fontFamily:"Inter,sans-serif"},children:["\\u2191",_deps[W.id].blocking.length]}),'
    '_deps[W.id].blocked.length>0&&e.jsxs("span",{style:{fontSize:10,fontWeight:700,color:"#1a56db",'
    'fontFamily:"Inter,sans-serif"},children:["\\u2193",_deps[W.id].blocked.length]})]}'
    '):e.jsx("span",{style:{fontSize:11,color:"#d1d5db",fontFamily:"Inter,sans-serif"},children:"—"})}),'
    # plus column cell
    'e.jsx("div",{style:{width:36,flexShrink:0}})'
)
_p4_old = 'children:[W.progress,"%"]})]}),]}),_e&&je.map'
# close span `})`, close cell-div `]})`, sep `,`, DEP_CELL, close flex-row `]})`, sep `,`, next sibling
_p4_new = 'children:[W.progress,"%"]})]}),' + DEP_CELL + ']}),_e&&je.map'
patches.append(('Add DEP+PLUS cells', _p4_old, _p4_new))

# ── 5. Inject dependency modal before filter panel ──────────────────────────
# Arrow functions throughout; Object.assign for state updates (no spread)
MODAL = (
    ...
    'onClick:()=>{_setDepItem(null);_setDepSrch("");}})'
    ...
    'e.jsxs("div",{children:['
    'e.jsx("h2",{style:{fontSize:15,fontWeight:700,color:"#111827",fontFamily:"Inter,sans-serif",margin:0},children:"Dependencies"}),'
    'e.jsx("p",{style:{fontSize:11,color:"#6b7280",fontFamily:"Inter,sans-serif",marginTop:3,marginBottom:0},'
    'children:"Define how this entity blocks or is blocked by others"})]}),'
    'e.jsx("button",{onClick:()=>{_setDepItem(null);_setDepSrch("");},style:{'
    ...
    'textTransform:"uppercase",fontFamily:"Inter,sans-serif",marginBottom:10},children:"DEPENDENCY TYPE"}),'
    ...
    'fontFamily:"Inter,sans-serif",cursor:"pointer",textAlign:"left"},children:"Finish-to-Start"}),'
    ...
    ',e.jsx("p",{style:{fontSize:10,color:"#9ca3af",fontFamily:"Inter,sans-serif",marginTop:8,marginBottom:0},'
    ...
    'e.jsxs("button",{onClick:()=>_setDepBkOpen(o=>!o),style:{'
    ...
    'e.jsx("span",{style:{fontSize:13,fontWeight:600,color:"#111827",fontFamily:"Inter,sans-serif"},children:"Blocking"}),'
    ...
    'e.jsx(al,{size:13,style:{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#9ca3af"}}),'
    'e.jsx("input",{value:_depSrch,onChange:c=>_setDepSrch(c.target.value),'
    'placeholder:"Search entities...",style:{width:"100%",paddingLeft:30,paddingRight:10,'
    'paddingTop:7,paddingBottom:7,borderRadius:8,border:"1px solid #e5e7eb",fontSize:12,'
    'fontFamily:"Inter,sans-serif",color:"#111827",outline:"none",boxSizing:"border-box"}})]}),'
    'Z.filter(it=>it.id!==_depItem.id&&(!_depSrch||it.name.toLowerCase().includes(_depSrch.toLowerCase()))).map(it=>{'
    'const _s=(_deps[_depItem.id]||{blocking:[]}).blocking.includes(it.id);'
    'const _tc=it.type==="Activity"?"#7c3aed":it.type==="Sub-Task"?"#d97706":"#1a56db";'
    'const _c=_deps[_depItem.id]||{blocking:[],blocked:[]};'
    'const _nb=_s?_c.blocking.filter(id=>id!==it.id):_c.blocking.concat([it.id]);'
    'return e.jsxs("div",{onClick:()=>_setDeps(prev=>Object.assign({},prev,{[_depItem.id]:Object.assign({},_c,{blocking:_nb})})),'
    'style:{display:"flex",alignItems:"center",gap:8,padding:"7px 0",cursor:"pointer",borderBottom:"1px solid #f9fafb"},children:['
    'e.jsx("div",{style:{width:16,height:16,borderRadius:4,'
    'border:_s?"2px solid #1a56db":"2px solid #e5e7eb",'
    'background:_s?"#1a56db":"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0},'
    'children:_s?e.jsx("span",{style:{color:"#fff",fontSize:9,fontWeight:700},children:"✓"}):null}),'
    'e.jsx("span",{style:{fontSize:9,fontWeight:600,color:_tc,background:_tc+"18",borderRadius:4,'
    'padding:"1px 5px",flexShrink:0,fontFamily:"Inter,sans-serif"},children:(it.type||"Task").toUpperCase()}),'
    ...
    ']},it.id);})]})]})]}),'
    # Note: the .map() callback above references _c and _nb which are computed per-row.
    # They need to be inside the onClick handler. Let me fix this:
)

# Actually the above has a bug: _c and _nb are computed outside onClick but reference _depItem.id.
# Need to rewrite with them inside the onClick arrow function.
# Rewrite blocking section map callback properly:
MODAL = (
    '_depItem&&e.jsxs("div",{style:{position:"fixed",inset:0,zIndex:200,'
    'display:"flex",alignItems:"center",justifyContent:"center",'
    'background:"rgba(0,0,0,0.45)",padding:"20px"},children:['
    ...
    'fontFamily:"Inter,sans-serif",cursor:"pointer",textAlign:"left"},children:"Finish-to-Start"}),'
    'e.jsxs("button",{style:{padding:"8px 12px",borderRadius:8,border:"1px solid #e5e7eb",'
    'background:"#f9fafb",fontSize:12,color:"#9ca3af",fontFamily:"Inter,sans-serif",'
    'cursor:"not-allowed",textAlign:"left",display:"flex",alignItems:"center",justifyContent:"space-between"},'
    'children:["Finish-to-Finish",e.jsx("span",{children:"\U0001f512"})]}),'
    'e.jsxs("button",{style:{padding:"8px 12px",borderRadius:8,border:"1px solid #e5e7eb",'
    'background:"#f9fafb",fontSize:12,color:"#9ca3af",fontFamily:"Inter,sans-serif",'
    'cursor:"not-allowed",textAlign:"left",display:"flex",alignItems:"center",justifyContent:"space-between"},'
    ...
    # scrollable body
    'e.jsxs("div",{style:{flex:1,overflowY:"auto"},children:['
    # blocking section header + list
    'e.jsxs("div",{style:{borderBottom:"1px solid #f0f1f4"},children:['
    'e.jsxs("button",{onClick:()=>_setDepBkOpen(o=>!o),'
    ...
    'e.jsxs("div",{style:{position:"relative",marginBottom:10},children:['
    'e.jsx(al,{size:13,style:{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#9ca3af"}}),'
    'e.jsx("input",{value:_depSrch,onChange:c=>_setDepSrch(c.target.value),placeholder:"Search entities...",'
    ...
    'const _s=(_deps[_depItem.id]||{blocking:[]}).blocking.includes(it.id);'
    'const _tc=it.type==="Activity"?"#7c3aed":it.type==="Sub-Task"?"#d97706":"#1a56db";'
    'return e.jsxs("div",{onClick:()=>{const _c=_deps[_depItem.id]||{blocking:[],blocked:[]};'
    '_setDeps(Object.assign({},_deps,{[_depItem.id]:Object.assign({},_c,{blocking:_s?_c.blocking.filter(id=>id!==it.id):_c.blocking.concat([it.id])})}));},style:{'
    'display:"flex",alignItems:"center",gap:8,padding:"7px 0",cursor:"pointer",borderBottom:"1px solid #f9fafb"},children:['
    'e.jsx("div",{style:{width:16,height:16,borderRadius:4,'
    'border:_s?"2px solid #1a56db":"2px solid #e5e7eb",'
    'background:_s?"#1a56db":"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0},'
    'children:_s?e.jsx("span",{style:{color:"#fff",fontSize:9,fontWeight:700},children:"✓"}):null}),'
    ...
    'padding:"1px 5px",flexShrink:0,fontFamily:"Inter,sans-serif"},children:(it.type||"Task").toUpperCase()}),'
    'e.jsx("span",{style:{fontSize:12,color:"#111827",fontFamily:"Inter,sans-serif",'
    'flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"},children:it.name})]},it.id);})]})]})]}),'
    # blocked section header + list
    'e.jsxs("div",{children:['
    'e.jsxs("button",{onClick:()=>_setDepBdOpen(o=>!o),'
    'style:{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",'
    'padding:"12px 20px",background:"none",border:"none",cursor:"pointer"},children:['
    'e.jsxs("div",{style:{display:"flex",alignItems:"center",gap:8},children:['
    '_depBdOpen?e.jsx(qe,{size:13,style:{color:"#9ca3af"}}):e.jsx(Oe,{size:13,style:{color:"#9ca3af"}}),'
    'e.jsx("span",{style:{fontSize:13,fontWeight:600,color:"#111827",fontFamily:"Inter,sans-serif"},children:"Blocked"}),'
    'e.jsx("span",{style:{fontSize:11,color:"#9ca3af",fontFamily:"Inter,sans-serif"},'
    ...
    'children:Z.filter(it=>it.id!==_depItem.id&&(!_depSrch||it.name.toLowerCase().includes(_depSrch.toLowerCase()))).map(it=>{'
    'const _s=(_deps[_depItem.id]||{blocked:[]}).blocked.includes(it.id);'
    'const _tc=it.type==="Activity"?"#7c3aed":it.type==="Sub-Task"?"#d97706":"#1a56db";'
    'return e.jsxs("div",{onClick:()=>{const _c=_deps[_depItem.id]||{blocking:[],blocked:[]};'
    '_setDeps(Object.assign({},_deps,{[_depItem.id]:Object.assign({},_c,{blocked:_s?_c.blocked.filter(id=>id!==it.id):_c.blocked.concat([it.id])})}));},style:{'
    'display:"flex",alignItems:"center",gap:8,padding:"7px 0",cursor:"pointer",borderBottom:"1px solid #f9fafb"},children:['
    'e.jsx("div",{style:{width:16,height:16,borderRadius:4,'
    'border:_s?"2px solid #1a56db":"2px solid #e5e7eb",'
    'background:_s?"#1a56db":"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0},'
    ...
    'fontSize:13,fontWeight:600,color:"#374151",fontFamily:"Inter,sans-serif",cursor:"pointer"},children:"Cancel"}),'
    'e.jsx("button",{onClick:()=>{_setDepItem(null);_setDepSrch("");},style:{'
    'padding:"9px 20px",borderRadius:8,border:"none",background:"#1a56db",'
    'fontSize:13,fontWeight:600,color:"#fff",fontFamily:"Inter,sans-serif",cursor:"pointer"},children:"Done"})]})]})]}),'
)
